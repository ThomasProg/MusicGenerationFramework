...
shouldPlot = False
resume_from_checkpoint = "Assets/Models/ddpm-butterflies-32/unet/diffusion_pytorch_model.safetensors"
...
class TrainingConfig:
    ...
    output_dir = 'Assets/Models/ddpm-butterflies-16'  # the model namy locally and on the HF Hub
...
# dataloader = torch.utils.data.DataLoader(load_transformed_dataset(), batch_size=128, shuffle=True, drop_last=True)
...
# dataset_name = "single_image_dataset"
# dataset_description = "A dataset containing only one image"
# dataset_citation = "Optional: citation for the dataset"

# # Step 3: Create dataset dictionary
# dataset_dict = {
#     "image": ["Assets/Datasets/Flowers102/flowers-102/jpg/image_00001.jpg"],  # Replace with the path to your image
#     "labels": [0],  # Optional: Add label if needed
# }
...
# def transform(examples):
#     images = [preprocess2(image.convert("RGB")) for image in examples["image"]]
#     return {"images": images}

# dataset2.set_transform(transform)

# # Step 5: Optional - Add dataset information
# # dataset.description = dataset_description
# # dataset.citation = dataset_citation

# dataloader = torch.utils.data.DataLoader(dataset2, batch_size=128, shuffle=True, drop_last=True)
# # train_dataloader = dataloader
...
def load_checkpoint(model, optimizer, lr_scheduler, checkpoint_path):
    checkpoint = torch.safe_load(checkpoint_path)
...
def train_loop(config, model, noise_scheduler, optimizer: torch.optim.Optimizer, train_dataloader, lr_scheduler):
    ...
    if resume_from_checkpoint and os.path.isdir(config.output_dir):
        # pipeline = DDPMPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", use_safetensors=True)
        # print(pipeline)
        # model = pipeline.unet
        # print(pipeline.config)
        # start_epoch = pipeline.config["currentEpoch"]
        ...
        # model, optimizer, lr_scheduler, start_epoch = load_checkpoint(model, optimizer, lr_scheduler, resume_from_checkpoint)
        ...
        if accelerator.is_main_process:
            ...
            if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
                if config.push_to_hub:
                    ...
                    torch.save(optimizer.state_dict(), os.path.join(config.output_dir, "optimizer_state_dict.pt"))
                    torch.save(lr_scheduler.state_dict(), os.path.join(config.output_dir, "lr_scheduler_state_dict.pt"))
                    

                    # Save epoch information in configuration
                    config_path = os.path.join(config.output_dir, "config.json")
                    customConfig = {
                        "epoch": epoch
                    }
                    with open(config_path, "w") as f:
                        json.dump(customConfig, f)
